...
import com.test.IndexBar.bean.BaseIndexTagBean;
...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private int mTitleHeight;//title的高,也就是A,B,C,D。。。。这些东西的高
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final RecyclerView.LayoutParams params = (RecyclerView.LayoutParams) child
                    .getLayoutParams();
            int position = params.getViewLayoutPosition(); // 获取item项在列表上的位置，从0开始
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
        ...
        // item 没有设置MarginTop的情况下，等同于下面的效果，设置了以后，就必须用上面的drawRect
